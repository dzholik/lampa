...
function component(object){
    ...
    let filter_sources = ['videocdn','rezka','kinobase','collaps','filmix','vibix']
    let ignore_sources = ['filmix','kinobase','vibix']
    let kiposk_sources = ['rezka','collaps','vibix']
    ...
    this.find = function(){
        // Для Vibix используем прямой поиск с правильным ID
        if(balanser === 'vibix'){
            this.extendChoice()
            // Передаем любой доступный ID, vibix.js сам определит тип
            let vibix_id = object.movie.kinopoisk_id || object.movie.imdb_id
            console.log('Vibix search ID:', vibix_id, 'KP:', object.movie.kinopoisk_id, 'IMDB:', object.movie.imdb_id)
