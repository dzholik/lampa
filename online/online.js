...
const button = `<div class="full-start__button selector view--online" data-subtitle="v1.55">
...
Lampa.Params.select('online_proxy_vibix','','')
...
Lampa.Template.add('settings_proxy',`<div>
    ...
    <div class="settings-param selector" data-type="input" data-name="online_proxy_vibix" placeholder="#{online_proxy_placeholder}">
        <div class="settings-param__name">Vibix</div>
        <div class="settings-param__value"></div>
    </div>
</div>`)
...
function addSettingsVibix() {
    if (Lampa.Settings.main && !Lampa.Settings.main().render().find('[data-component="vibix"]').length) {
        ...
        let after = Lampa.Settings.main().render().find('[data-component="filmix"]')
        if (after.length) {
            after.after(field)
        } else {
            Lampa.Settings.main().render().find('[data-component="more"]').after(field)
